...
export interface Profile {
  ...
  trip_notes:       string | null   // preferenze libere per il prossimo viaggio
  created_at:       string
  updated_at:       string
}
...
export interface Trip {
  ...
  created_at:  string
  updated_at:  string
  packing_template?: string[] | null   // Modulo P: lista AI generata una volta
}
...
export interface Day {
  ...
  position:   number
  lat:        number | null   // coordinate città (mappa itinerario)
  lng:        number | null
  created_at: string
  updated_at: string
}
...
// ── Modulo O: Bacheca Note & Task ─────────────────────────────
export interface GroupBoardItem {
  id:           string
  trip_id:      string
  created_by:   string
  content_type: 'nota' | 'task' | 'packing'   // packing: personale (Modulo P)
  text_content: string
  ...
  completer?:   Pick<Profile, 'id' | 'username' | 'full_name'>
}

// ── Modulo P: Documenti & Prenotazioni ────────────────────────
export type DocType = 'volo' | 'hotel' | 'treno' | 'bus' | 'noleggio' | 'biglietto' | 'assicurazione' | 'altro'
...
export interface TravelerProfile {
  ...
  generated_at:      string
  // Psicologo V2 (migration 020)
  pace_preference:   number | null
  social_openness:   number | null
  novelty_seeking:   number | null
  mobility_level:    'full' | 'moderate' | 'limited' | null
  travel_style:      'planner' | 'spontaneous' | 'mixed' | null
  language_comfort:  'local_only' | 'english_ok' | 'multilingual' | null
  pace_note:         string | null
}

// ── Modulo N: Passaporto del Viaggiatore ─────────────────────
...
export type Database = {
  public: {
    Tables: {
      ...
      user_visited_countries: { Row: VisitedCountry;   Insert: Omit<VisitedCountry, 'created_at'>;         Update: Partial<Pick<VisitedCountry, 'visited_at'>> }
    }
    Functions: {
