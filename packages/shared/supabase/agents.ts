...
import { buildEnricherContext } from './enricher'
import type { EnricherOutput } from './enricher'
...
export interface ActivitySuggestion {
  ...
  group_fit_reason?: string  // perché questo suggerimento è adatto a questo gruppo specifico
  activity_data?: {
    title:      string
    notes:      string | null
    location:   string | null
    time_start: string | null
    date:       string | null  // YYYY-MM-DD: data consigliata per l'attività
  }
}
...
export interface TravelerProfileOutput {
  ...
  raw_analysis:      string
  // Nuovi
  pace_preference:   number                                    // 1-5
  social_openness:   number                                    // 1-5
  novelty_seeking:   number                                    // 1-5
...
// Input: conflitti meteo + itinerario + profili viaggiatori (opzionali)
// Output: suggerimenti prioritizzati e personalizzati
...
export async function runTravelPlannerWeatherAgent(
  ...
  forecasts:         DayForecast[],
  travelerProfiles?: TravelerProfileOutput[],  // Modulo K: personalizzazione opzionale
  enricherOutput?:   EnricherOutput,           // Agente 4: dati reali Foursquare
...
Restituisci SOLO JSON valido.`
  ...
  // Fasce orarie sicure per spostare attività outdoor
  ...
  "suggestions": [
    {
      ...
      "activity_data": {
        ...
        "date": "YYYY-MM-DD — data in cui svolgere l'attività (scegli una fascia meteo ok)"
      }
    }
  ]
}`
...
function validatePsychologistOutput(raw: unknown): TravelerProfileOutput {
  ...
  if (!['full', 'moderate', 'limited'].includes(p.mobility_level as string))
    p.mobility_level = 'moderate'
  if (!['planner', 'spontaneous', 'mixed'].includes(p.travel_style as string))
    p.travel_style = 'mixed'
  if (!['local_only', 'english_ok', 'multilingual'].includes(p.language_comfort as string))
    p.language_comfort = 'english_ok'
  if (!Array.isArray(p.personality_tags) || p.personality_tags.length === 0)
    p.personality_tags = ['explorer']
  if (typeof p.raw_analysis !== 'string' || p.raw_analysis.trim() === '')
    p.raw_analysis = 'Profilo non disponibile.'
  if (typeof p.pace_note !== 'string' || p.pace_note.trim() === '')
    p.pace_note = 'Ritmo standard, equilibrio tra attività e riposo.'
...
// Attivato quando la timeline ha tappe ma zero attività.
// Genera un piano giornaliero completo per ogni tappa.
...
${travelerProfiles!.map((p, i) =>
  ...
  // Rileva giorni di transizione: quando date_end di tappa[i] == date di tappa[i+1]
  // In quei giorni il gruppo si sposta in macchina tra le due città
  ...
  const transitionDates = new Set(transitions.map(t => t.date))
...
// Genera una checklist valigia personalizzata su destinazione,
// durata del viaggio e clima tipico del periodo (mese/stagione).
// Output: array di stringhe (ogni voce con emoji iniziale).
...
const PACKING_FALLBACK = [
  ...
  '💊 Farmaci personali',
  '🪥 Necessaire (spazzolino, dentifricio…)',
  '👕 Cambi di vestiti',
  '🧥 Giacca adatta al meteo',
  '🕶️ Occhiali da sole / crema solare',
  '📱 SIM/eSIM o piano dati',
  '🎧 Auricolari',
  '🧴 Articoli da bagno',
]
