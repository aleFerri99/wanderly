...
import { runMeteorologoAgent, runTravelPlannerWeatherAgent } from '@repo/shared/supabase/agents'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // ── 0. Pruning: rimuove le previsioni passate (non più utili) ─
    // Mantiene weather_cache limitato a date odierne/future → contiene
    // la crescita su DB (free tier 500MB).
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabase as any).from('weather_cache').delete().lt('forecast_date', todayStr)
